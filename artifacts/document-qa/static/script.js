...
function setChatEnabled(enabled) {
  ...
  if (enabled) {
    chatSubtitle.textContent = "Ready! Ask me anything.";
  }
}
...
pdfInput.addEventListener("change", async () => {
  ...
  try {
    ...
    setUploadStatus("ready", `${data.filename} uploaded`);
...
chatForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    removeTypingIndicator();
    const bubble = appendMessage("ai", error.message || "Something went wrong.");
    bubble.querySelector(".bubble").classList.add("bubble-error");
  } finally {
